...
export const useVolunteerStore = create<VolunteerState>((set, get) => ({
  ...
  fetchAll: async () => {
    try {
      set({ loading: true });
      await Promise.all([get().fetchLicense(), get().fetchRequests(), get().fetchEvaluations()]);
    } catch (e: unknown) {
  ...
  setLicense: async (l) => {
    set({ license: l });
    localStorage.setItem(STORAGE_LICENSE, await encrypt(JSON.stringify(l)));
  },
  ...
  fetchLicense: async () => {
    try {
      ...
      localStorage.setItem(STORAGE_LICENSE, await encrypt(JSON.stringify(data[0])));
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : "Unknown error");
    }
  },
    ...
    try {
      ...
      set({
        requests: get().requests.map((r) =>
          r.requestId === requestId ? { ...r, requestStatus: data.requestStatus } : r
        ),
      });
